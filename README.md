...
**KireNode** 是一个专为 Linux VPS（如 Ubuntu）设计的智能 VPN 代理网关管理器。它能够自动采集 VPNGate 开放节点，进行多线程可用性测试与延迟过滤，利用 OpenVPN 隧道与策略路由（Policy Routing）实现出站网络，并在本地提供高性能的 HTTP/SOCKS5 代理网关服务，适合用作 Xray 的落地出站代理。
...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/Special-Care/Kire-Node/main/install.sh)
```
...
安装成功后，系统会在全局注册 `kire` 快捷管理指令，直接运行 `kire` 可打开图形化交互终端，也可通过以下指令执行：
* **`kire status`** 或 **`kire`**：查看当前运行状态（代理端口、活动 VPN 节点、直连延迟、网页后台登录地址等）。
* **`kire start`**：启动 KireNode 服务。
* **`kire stop`**：停止 KireNode 服务（并自动清理策略路由与 OpenVPN 进程）。
* **`kire restart`**：重启服务。
* **`kire logs`**：查看实时的 Systemd 服务运行日志。
* **`kire web`**：切换网页绑定地址（127.0.0.1 仅本地，或 0.0.0.0 允许公网访问）与重置安全后缀。
* **`kire port`**：修改网页管理控制台监听端口。
* **`kire password`**：生成新的 12 位安全管理密码。
* **`kire uninstall`**：完全卸载服务并清理相关环境。
...
```bash
curl -Ls https://raw.githubusercontent.com/Special-Care/Kire-Node/main/install.sh -o install.sh
...
**KireNode** is an intelligent VPN proxy gateway manager designed specifically for Linux VPS (e.g. Ubuntu). It automatically collects open VPNGate nodes, conducts multi-threaded availability testing and latency filtering, establishes secure out-of-band routing via OpenVPN and policy routing to **prevent VPS lockouts**, and hosts a high-performance local SOCKS5/HTTP proxy gateway. It is highly optimized to serve as a residential/unlocked egress node for upstream proxies like 3x-ui / Xray.
...
5. 🛠️ **CLI Utility (kire)**:
   * Command-line helper tool `kire` with a menu-driven interface.
...
To install and deploy KireNode on your **Ubuntu** server, copy and paste the following command:
...
Once installed, use the global command `kire` to launch the interactive helper menu, or use the shortcuts below:
* **`kire status`** or **`kire`**: Check running system status (active nodes, proxy ports, latency, URLs).
* **`kire start`**: Start the gateway service.
* **`kire stop`**: Stop the gateway service (and clean routing tables).
* **`kire restart`**: Restart the service.
* **`kire logs`**: View real-time Systemd output logs.
* **`kire web`**: Toggle Web UI accessibility (127.0.0.1 or 0.0.0.0) and reset suffix paths.
* **`kire port`**: Update the Web Console port.
* **`kire password`**: Regenerate a secure 12-character administration password.
* **`kire uninstall`**: Completely remove the service and repository files from your VPS.
