...
if [ -f /etc/os-release ]; then
    . /etc/os-release
    if [[ "${ID:-}" != "ubuntu" ]]; then
        echo -e "${RED}错误: 本系统不是 Ubuntu！目前 KireNode 仅支持 Ubuntu 系统。${PLAIN}"
        exit 1
    fi
else
...
echo -e "${BLUE}==========================================================${PLAIN}"
echo -e "${BLUE}        欢迎使用 KireNode 一键源码部署与管理脚本${PLAIN}"
echo -e "${BLUE}==========================================================${PLAIN}"

# 3. Configure GitHub Repository URL
# Default to the official repository (Special-Care/Kire-Node)
DEFAULT_USER="Special-Care"
DEFAULT_REPO="Kire-Node"
...
echo -e "  -> 正在创建服务配置 /lib/systemd/system/kire-node.service ..."
cat > /lib/systemd/system/kire-node.service <<EOF
[Unit]
Description=KireNode OpenVPN Manager with HTTP/SOCKS5 Proxy
...
Restart=always
RestartSec=5
EnvironmentFile=-/etc/default/kire-node
...
systemctl enable kire-node.service

# 6. Configure global command shortcut "kire"
echo -e "\n${YELLOW}[4/4] 正在创建全局命令快捷接口 'kire'...${PLAIN}"
echo -e "  -> 正在写入管理脚本 /usr/bin/kire ..."
cat > /usr/bin/kire <<'EOF'
#!/usr/bin/env python3
import sys
import os
import socket
...
INSTALL_DIR = "/opt/kire-node"
LOG_FILE = "/opt/kire-node/vpngate_data/vpngate.log"
...
def load_state():
    import json
    path = "/opt/kire-node/vpngate_data/state.json"
...
def get_active_node_info():
    import json
    path = "/opt/kire-node/vpngate_data/nodes.json"
    state = load_state()
...
def get_public_ip():
    path = "/opt/kire-node/vpngate_data/public_ip.txt"
    if os.path.exists(path):
        try:
            with open(path, "r", encoding="utf-8") as f:
                ip = f.read().strip()
...
def get_service_pid(service_name="kire-node.service"):
    try:
...
def check_service_active(service_name="kire-node.service"):
...
def start_service():
    print("正在启动 KireNode 服务...", flush=True)
    subprocess.run(["systemctl", "start", "kire-node.service"])
    print("已发送启动指令。")
    time.sleep(1)

def stop_service():
    print("正在停止 KireNode 服务...", flush=True)
    subprocess.run(["systemctl", "stop", "kire-node.service"])
    print("已发送停止指令。")
    time.sleep(1)

def restart_service():
    print("正在重启 KireNode 服务...", flush=True)
    subprocess.run(["systemctl", "restart", "kire-node.service"])
    print("已发送重启指令。")
    time.sleep(1)

def show_logs():
    print("正在查看 KireNode 日志 (按 Ctrl+C 退出)...", flush=True)
...
def uninstall_service():
    confirm = input("确定要完全卸载 KireNode 吗？(y/N): ")
    if confirm.lower() == 'y':
        print("正在完全卸载 KireNode...", flush=True)
        subprocess.run(["systemctl", "stop", "kire-node.service"])
        subprocess.run(["systemctl", "disable", "kire-node.service"])
        try:
            os.unlink("/lib/systemd/system/kire-node.service")
        except Exception:
            pass
        try:
            os.unlink("/usr/bin/kire")
        except Exception:
            pass
...
def main():
    ...
    options = {
        '1': ("启动服务 (kire start)", start_service),
        '2': ("停止服务 (kire stop)", stop_service),
        '3': ("重启服务 (kire restart)", restart_service),
        '4': ("日志监控 (kire logs)", show_logs),
        '5': ("网页配置 (kire web)", configure_web),
        '6': ("端口配置 (kire port)", configure_port),
        '7': ("账号密码 (kire password)", configure_credentials),
        '8': ("一键更新 (kire update)", update_service),
        '9': ("完全卸载 (kire uninstall)", uninstall_service),
        '0': ("退出终端", None)
    }
...
# 8. Start service
echo -e "\n正在启动 KireNode 服务并初始化网络..."
systemctl restart kire-node.service || true

# Wait and poll for node loading and active connection
echo -e "\n正在等待 KireNode 首次获取节点并建立加密通道 (此过程可能需要 5-30 秒)..."
ACTIVE_ID=""
LAST_MSG=""
for i in {1..90}; do
...
echo -e "\n${GREEN}==========================================================${PLAIN}"
echo -e "${GREEN}             KireNode 源码一键部署已完成！${PLAIN}"
echo -e "${GREEN}==========================================================${PLAIN}"
